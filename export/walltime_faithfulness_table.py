    ...
    if args.stage in ['both', 'plot']:
        ...
        with open(args.persistent_dir / 'tables' / args.format / f'{output_name}.tex', 'w') as fp:
            ...
            for dataset_name, dur in df_tab.iterrows():
                roberta_sb = tex_format_time(dur.loc["durations.evaluate_mean", :].loc["roberta-sb"])
                roberta_sl = tex_format_time(dur.loc["durations.evaluate_mean", :].loc["roberta-sl"])
                print(f'{annotation.dataset.labeller(dataset_name)} & {roberta_sb} & {roberta_sl} \\\\', file=fp)

            print(r'\midrule', file=fp)
            print(r'\midrule', file=fp)

            roberta_sb = df_tab_total.loc["durations.evaluate_mean", :].loc["roberta-sb"]
            roberta_sl = df_tab_total.loc["durations.evaluate_mean", :].loc["roberta-sl"]
