...
if __name__ == "__main__":
    ...
    if args.stage in ['both', 'plot']:
        ...
        with open(args.persistent_dir / 'tables' / args.format / f'{output_name}.tex', 'w') as fp:
            print(r'\begin{tabular}{lcc}', file=fp)
            ...
            for dataset_name, dur in df_tab.iterrows():
                roberta_sb = dur.loc["durations.total_mean", :].loc["roberta-sb"]
                roberta_sl = dur.loc["durations.total_mean", :].loc["roberta-sl"]

                print(f'{annotation.dataset.labeller(dataset_name)} & {tex_format_time(roberta_sb)} & {tex_format_time(roberta_sl)} \\\\', file=fp)
            print(r'\midrule', file=fp)
            print(r'\midrule', file=fp)
